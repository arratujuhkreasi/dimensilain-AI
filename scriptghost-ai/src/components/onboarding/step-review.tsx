...
import { SUB_GENRE_LABELS, DURATION_CONFIG, JUMP_SCARE_LABELS } from "@/lib/utils/duration-config";
...
export function StepReview() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold">Tinjau Proyek</h2>
      ...
      <div className="space-y-4 text-sm">
        <div className="p-4 rounded-lg border border-border space-y-2">
          ...
          <p className="text-muted-foreground">{config.logline || "Belum ada inti cerita"}</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="p-3 rounded-lg bg-muted">
            ...
            <span className="font-medium">
              {durConfig.acts} babak x {durConfig.scenesPerAct} adegan
            </span>
          </div>
          <div className="p-3 rounded-lg bg-muted">
            <span className="text-muted-foreground">Horor:</span>{" "}
            <span className="font-medium">{JUMP_SCARE_LABELS[config.jumpScareDensity]}</span>
          </div>
        </div>
      ...
      {!validation.success && (
        <p className="text-center text-xs text-muted-foreground">
          Lengkapi judul, inti cerita, lokasi, dan minimal satu nama karakter sebelum naskah dibuat.
        </p>
      )}
    </div>
  );
